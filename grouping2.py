...
def group_users(users_dict):
   ...
   users = [(memberId, [(review["productId"], review["Rate"]) for review in reviews]) for memberId, reviews in users_dict.items()]
   groups = {}
   for i in range(len(users)-1):
      ref_user = users[i]
      for j in range(i+1,len(users)):
         comp_user = users[j]
         common_products = set(ref_user[1]).intersection(set(comp_user[1]))
         common_products = sorted([review[0] for review in common_products])
         if len(common_products) >= 3:
            key = "-".join(common_products)
...
# with open("./library/groups.txt", "w") as f:
